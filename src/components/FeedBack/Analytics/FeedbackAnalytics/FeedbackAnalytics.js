...
const StrengthComponent = ({ item }) => {
  ...
  Object.entries(item?.response).forEach(([question, responseValue], index) => {
    if (
      ...
      (responseValue / item.response.total).toFixed(2) >= 4
    ) {
...
const WeeknessComponent = ({ item }) => {
  ...
  Object.entries(item?.response).forEach(([question, responseValue], index) => {
    if (
      question !== "total" &&
      question.includes("sum") &&
      (responseValue / item.response.total).toFixed(2) < 3
    ) {
      strengths.push(question1[index - 1]);
    }
  });
...
{      reportType==="1"?      <div className={styles.responsesContainer}>
              <div className={`${styles.tableContainer} pagebreak`}>
                <table>
                  ...
                  <tbody>
                    <tr>
                      {Object.entries(item?.response).map(
                        ([question, responseValue]) =>
                          question !== "total" &&
                          question.includes("sum") && (
                            <td key={question}>
                              {(responseValue / item.response.total).toFixed(2)}
                            </td>
                          )
                      )}
                    </tr>
              ...
              <div className={`${styles.yesNoContainer} pagebreak`}>
                ...
                <div className={styles.questionyesno}>
                  ...
                  <div className={styles.q}>
                    Result of test declared within two weeks of it being
                    conducted
                  </div>
                  <div className={styles.yesno}>
                    {(
                      (item.response.q11_count / item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                  </div>
                  <div className={styles.yesno}>
                    {" "}
                    {(
                      ((item.response.total - item.response.q11_count) /
                        item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                ...
                <div className={styles.questionyesno}>
                  ...
                  <div className={styles.yesno}>
                    {(
                      (item.response.q12_count / item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                  </div>
                  <div className={styles.yesno}>
                    {(
                      ((item.response.total - item.response.q12_count) /
                        item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                ...
                <div className={styles.questionyesno}>
                  ...
                  <div className={styles.q}>
                    Would you recommend him/her to teach the same subject to
                    your juniors?
                  </div>
                  <div className={styles.yesno}>
                    {(
                      (item.response.q13_count / item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                  </div>
                  <div className={styles.yesno}>
                    {" "}
                    {(
                      ((item.response.total - item.response.q13_count) /
                        item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                ...
                <div className={styles.questionyesno}>
                  ...
                  <div className={styles.yesno}>
                    {(
                      (item.response.q14_count / item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                  </div>
                  <div className={styles.yesno}>
                    {" "}
                    {(
                      ((item.response.total - item.response.q14_count) /
                        item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                ...
                <div className={styles.questionyesno}>
                  ...
                  <div className={styles.yesno}>
                    {(
                      (item.response.q15_count / item.response.total) *
                      100
                    ).toFixed(2)}
                    %
                  </div>
                  <div className={styles.yesno}>
                    {" "}
                    {(
                      ((item.response.total - item.response.q15_count) /
                        item.response.total) *
                      100
                    ).toFixed(2)}
                    %
              ...
              <div className={styles.overAllContainer}>
                <div>
                  Strength :<StrengthComponent item={item} />
                </div>

                <div>
                  Weekness : <WeeknessComponent item={item} />
                </div>
